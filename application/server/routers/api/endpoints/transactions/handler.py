...
router = APIRouter()  # TODO: add checking current user
...
async def get_transaction_by_id(
    ...
    """
    transaction = await transaction_service.transaction_repo.get_by_id(
...
async def create_transaction(
    *,
    ...
    """
    from_account = await account_service.account_repo.get_by_id(
        instance_id=transaction_in.from_account_id
    )
    if not from_account:
    ...
    to_account = await account_service.account_repo.get_by_id(
        instance_id=transaction_in.to_account_id
    )
    if not to_account:
        raise HTTPException(  # TODO: move checking into services
            status_code=400,
    ...
    transaction = await transaction_service.register_transaction(
        from_account, to_account, transaction_amount
    )
    background_tasks.add_task(
        evaluate_transaction, transaction, account_service, transaction_service
    )
    return transaction
